...
def helix_maker(radius, revolutions, pitch, resolution, r1, DT1, SP1, accumulator, z1, DT2, SP2, plane, points_old):
    ...
    preview = True
    try:
        fnR = Expression(r1,["t", "r", "res"])
        fnR(3, pitch, resolution)  # test if equation is legal, if not use previous points (points_old)
        fnZ = Expression(z1,["t", "p", "res"])
        fnZ(3, pitch, resolution)
    except:
        preview = False
    ...
    if preview:
        ...
        for t in range(0, int(revolutions*resolution)+1):
            ...
            point, Z_previous, skip_, message_string = helix_point(radius, pitch, resolution, t, fnR, DT1, SP1, accumulator, fnZ, DT2, SP2, Z_previous, message_string)  #, ui_message)
            if not skip_:
                points.add(point)
...
class Helix_Advanced_Command(Fusion360CommandBase):
    ...
    def on_create(self, command, inputs):
        ...
        # Radius of the helix
        radius_input = adsk.core.ValueInput.createByReal(25.54)
        tabInputs1.addValueInput('radius', 'Radius', default_units, radius_input)

        # Pitch of the helix
        pitch_input = adsk.core.ValueInput.createByReal(25.54)
        ...
        Instruction = "<b><u>Instruction</u><br>"\
            ...
            + "r -</b> radius, <b>p -</b> pitch, <b>res -</b> resolution<br>"\
            + "<b>t</b> - steps value, (default value 0)<br>"\
            ...
            + "<b>Tr -</b> steps for t radius <br>"\
            + "<b>Tp -</b> steps for t pitch <br>"\
            + "<b>start counting at –</b> configure t starting point for counting<br>"\
            ...
            + "Output warning messages for the user."
